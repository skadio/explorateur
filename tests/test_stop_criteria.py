...
class ExplorationStoppingCriteriaTests(BaseTest):
    ...
    def test_max_iterations(self):
        ...
        sol_state = explorer.search(starting_state, max_moves=2, dot_file_path="tmp/test_dfs_1")
    ...
    def test_max_runtime(self):
        ...
        We set a max time of 1 seconds to make sure that not all nodes are explored through the dot file.
        """
        explorer = Explorateur(ExplorationType.DepthFirst(), self.seed)
        clauses = [(1, -2), (-1, -2), (2, 3), (-3, 2), (1, 4)]

        starting_state = SATState(clauses)
        sol_state = explorer.search(starting_state, max_runtime=1, dot_file_path="tmp/test_dfs_2")
